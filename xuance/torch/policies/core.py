...
class OneHotCategoricalActorNet(Module):
    """
    The actor network for onehot categorical policy, which outputs a distribution over all discrete onehot actions.
    ...
    def __init__(self,
                 ...
                 activation: Optional[ModuleType] = None,
                 device: Optional[Union[str, int, torch.device]] = None):
        super(OneHotCategoricalActorNet, self).__init__()
        layers = []
        input_shape = (state_dim,)
        for h in hidden_sizes:
            mlp, input_shape = mlp_block(input_shape[0], h, normalize, activation, initialize, device)
    ...
    def forward(self, x: Tensor, avail_actions: Optional[Tensor] = None):
        ...
        if avail_actions is not None:
            logits[avail_actions == 0] = -1e10
        self.dist = torch.distributions.OneHotCategoricalStraightThrough(
            logits=logits)
        return self.dist

class GaussianCriticNet(Module):
    """
    The actor network for Gaussian value, which outputs a distribution of value.
    ...
    def forward(self, x: Tensor):
        """
        Returns the stochastic distribution of value.
        Parameters:
            x (Tensor): The input tensor.
